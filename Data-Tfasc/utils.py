# -*- coding: utf-8 -*-
"""
Utility functions for Tfasc crawler - Web parsing
"""
import regex as re
import traceback
import requests
...
from config import wbinfo


def num_transformer(num):
    _num_dict = {'１': '1', '２': '2', '３': '3', '４': '4', '５': '5', '６': '6', '７': '7', '８': '8', '９': '9', '０': '0'}
    for k, v in _num_dict.items():
        num = num.replace(k, v)
    return num


def getSection():
    url = wbinfo['section_url']
    data = {
        "sort": "",
        "page": "1",
        "pageSize": "30",
...
def getBuzBidTime(data):
    ...
    for item in resp_json['Data']:
        if str(item['FileRoot']) != 'None':
            ...
            doc['estate_url'] = wbinfo['estate_url'] + '?id=' + str(item['Id'])
...
def bulletin_etl(raw_tb, regexp):
    def _etl(tb, _type):
        ...
        for row in tb:
            ...
            if not _start and _type in row:
                _start = True
            if '│備考' in row:
...
def getBulletin(document, number):
    ...
    regexp = "{}（(.+?)）".format(re.sub(r'\d+', r'\d+', number))
...
def parseBulletin(tfasc, mode='prod'):
    ...
    required_columns = ['document', '金服案號']
    missing_columns = [col for col in required_columns if col not in df.columns]

    if missing_columns:
        print(f"警告：缺少必要的欄位 {missing_columns}")
        print(f"可用的欄位：{list(df.columns)}")
        if 'document' not in df.columns and 'bulletin_url' in df.columns:
            df['document'] = df['bulletin_url']
            print("已將 bulletin_url 設定為 document 欄位")
        if '金服案號' not in df.columns and 'number' in df.columns:
            df['金服案號'] = df['number']
            print("已將 number 設定為 金服案號 欄位")

    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        print(f"錯誤：仍然缺少必要的欄位 {missing_columns}，無法繼續處理")
        return []

    df = df[(df['document'].notna()) & (df['document'] != '') &
            (df['金服案號'].notna()) & (df['金服案號'] != '')]

    if df.empty:
        print("警告：沒有有效的資料可以處理")
        return []
    ...
    for document, number in df_g.groups.keys():
        ...
        for i in range(retry):
            try:
                if len(str(document)) == 0 or len(str(number)) == 0:
                    break
                else:
                    result += _parse_bulletin(df, df_g, document, number)
                    break
            except Exception as e:
                print(f"處理 {document}, {number} 時發生錯誤: {e}")
                traceback.print_exc()
        _index += 1
    return result
