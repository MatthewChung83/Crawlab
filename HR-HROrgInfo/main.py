...
# Initialize logger
logger = get_logger('HR-HROrgInfo')


def run():
    """Main execution function"""
    logger.task_start("組織資料同步")
    ...
    try:
        ...
        logger.ctx.set_operation("fetch_orginfo_data")
        data = fetch_orginfo_data(session_id)
        if not data:
            logger.error("無法取得組織資料")
            logger.task_end(success=False)
            return False

        logger.info(f"取得組織資料: {len(data)} 筆")
...
def main():
    """Main entry point"""
    logger.info("HR-HROrgInfo 組織資料同步")
